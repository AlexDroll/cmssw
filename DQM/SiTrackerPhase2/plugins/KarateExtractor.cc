// -*- C++ -*-
//
// Package:    KarateExtractor
// Class:      KarateExtractor
//
/**\class KarateExtractor KarateExtractor.cc 
...
#include "DQM/SiTrackerPhase2/plugins/KarateExtractor.h"
...
// destructor
//
KarateExtractor::~KarateExtractor() {
...
// -- Analyze
//
void KarateExtractor::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
...
void KarateExtractor::fillITPixelDigiHistos(const edm::Handle<edm::DetSetVector<PixelDigi>> handle,
  ...
  for (typename edm::DetSetVector<PixelDigi>::const_iterator DSViter = digis->begin(); DSViter != digis->end();
    ...
    for (typename edm::DetSet<PixelDigi>::const_iterator di = DSViter->begin(); di != DSViter->end(); di++) {
      ...
      nDigi++;
      edm::LogInfo("KarateExtractor") << "  column " << col << " row " << row << std::dec << std::endl;
      ...
      edm::LogInfo("KarateExtractor") << " row " << row << " col " << col << " row_last " << row_last
...
void KarateExtractor::fillOTDigiHistos(const edm::Handle<edm::DetSetVector<PixelDigi>> handle,
                                                const edm::ESHandle<TrackerGeometry> gHandle) {
  const edm::DetSetVector<PixelDigi>* digis = handle.product();
...
// -- Book Histograms
//
void KarateExtractor::bookHistograms(DQMStore::IBooker& ibooker,
                                              edm::Run const& iRun,
...
//
void KarateExtractor::bookLayerHistos(DQMStore::IBooker& ibooker,
                                               unsigned int det_id,
  ...
  if (pos == layerMEs.end()) {
    ...
    edm::LogInfo("KarateExtractor") << " Booking Histograms in : " << folder_name.str();
...
DEFINE_FWK_MODULE(KarateExtractor);
